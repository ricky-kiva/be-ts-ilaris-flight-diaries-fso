...
const parseComment = (comment: unknown): string => {
  if (!comment || !isString(comment)) {
    throw new Error('Inccorect or missing comment');
  }
...
const parseVisibility = (visibility: unknown): Visiblity => {
...
// `text is string` is for "Type Guards", to infer TypeScript the parameter has that type
